...
<template>
  <div :key="item.id" relative class="lg:aspect-ratio-25/12 aspect-ratio-3/2" bg-black>
    <div
    ...
    >
      <Transition appear name="hero">
        <div v-show="mounted">
          ...
          <div v-if="trailer" class="py5 display-none lg:block text-center">
            <button flex="~ gap2" items-center p="x6 y3" bg="gray/15 hover:gray/20" transition :title="$t('Watch Trailer')" @click="playTrailer()">
              <div i-ph-play />
              {{ $t('Watch Trailer') }}
            </button>
          </div>
